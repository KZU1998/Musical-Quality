...
class ControladorUsuario extends ControladorBase
{
   ...
   {
      $sql = "INSERT INTO Usuario (idUsuario) VALUES (?);";
      $parametros = array($usuario->idUsuario);
   ...
   {
      $parametros = array($usuario->idUsuario,$usuario->id);
      $sql = "UPDATE Usuario SET idUsuario = ? WHERE id = ?;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      if(is_null($respuesta[0])){
         return true;
      }else{
